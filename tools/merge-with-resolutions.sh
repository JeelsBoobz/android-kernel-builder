...
#
# Resolutions file: lines "<path> = ours|theirs|manual|fixup" (# comments,
# blanks ignored). "fixup" copies <manual-dir>/<path> over a NON-conflicted
# file the merge left at its LTS-tip state (verified before overwrite;
# post-merge corrections like compat shims). "ours"/"theirs" check out that side whole (verified
...
if git -c merge.conflictStyle=merge "${GITID[@]}" merge --no-ff --no-edit \
  ...
  if grep -vE '^[[:space:]]*(#|$)' "$RESOL" | grep -vE '=[[:space:]]*(ours|theirs|manual|fixup)[[:space:]]*(#|$)' | grep -q .; then
    log "bad resolution lines:"; grep -vE '^[[:space:]]*(#|$)' "$RESOL" | grep -vE '=[[:space:]]*(ours|theirs|manual|fixup)[[:space:]]*(#|$)'; exit 2
  fi
...
# Fixups run on BOTH paths (clean merge or resolved conflicts): recorded
# content for files the merge did NOT conflict (e.g. compat shims). The
# worktree file must still equal its LTS-tip state (untouched by the
# merge); otherwise the fixup was generated against moved tips and must
# be regenerated by a human.
FIXUP_APPLIED=""
while IFS= read -r line; do
    line="${line%%#*}"
    [ -z "${line//[[:space:]]/}" ] && continue
    f="${line%%=*}"; m="${line#*=}"
    f="$(echo "$f" | tr -d '[:space:]')"; m="$(echo "$m" | tr -d '[:space:]')"
    [ "$m" = "fixup" ] || continue
    if grep -qx "$f" /tmp/mwr_conflicted.txt 2>/dev/null; then
      log "FAIL: $f is both conflicted and fixup (use ours/theirs/manual)"; exit 1
    fi
    [ -f "$MANDIR/$f" ] || { log "fixup missing content: $MANDIR/$f"; exit 2; }
    git diff --quiet "$LTS_TIP" -- "$f" || { log "FAIL: fixup target $f diverged from LTS tip (regenerate fixup)"; exit 1; }
    cp "$MANDIR/$f" "$f"
    git add -- "$f"
  ...
  # Universal gates run on both paths; commit + size floor only where a
  # commit is actually created (conflict path).
